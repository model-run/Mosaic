import type { EngineId, EngineRecipe, ModelEntry, RecipeStatus } from "./types";
...
export function getRecipeStatus(model: ModelEntry, engineId: EngineId): RecipeStatus {
  const recipe = model.engines[engineId];
  // "none" is the absence sentinel: the model has no recipe for that engine.
  return recipe?.status ?? "none";
}

export function getRecipe(model: ModelEntry, engineId: EngineId): EngineRecipe | undefined {
  return model.engines[engineId];
}
